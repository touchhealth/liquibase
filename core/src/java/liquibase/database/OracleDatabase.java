...
public class OracleDatabase extends AbstractDatabase {
	...
	/**
	 * Return an Oracle date literal with the same value as a string formatted
	 * using ISO 8601.
	 * <p/>
	 ...
	 * ISO8601 are supported: YYYY-MM-DD YYYY-MM-DDThh:mm:ss
	 */
	...
	public SqlStatement getSelectChangeLogLockSQL() throws JDBCException {
		return new RawSqlStatement((super.getSelectChangeLogLockSQL().getSqlStatement(this) + " for update")
				.toUpperCase());
	}

	public SqlStatement createFindSequencesSQL(String schema) throws JDBCException {
		return new RawSqlStatement("SELECT SEQUENCE_NAME FROM ALL_SEQUENCES WHERE SEQUENCE_OWNER = '"
	...
	public SqlStatement getViewDefinitionSql(String schemaName, String name) throws JDBCException {
		return new RawSqlStatement("SELECT TEXT FROM ALL_VIEWS WHERE upper(VIEW_NAME)='" + name.toUpperCase()
	...
	public Object convertDatabaseValueToJavaObject(Object defaultValue, int dataType, int columnSize, int decimalDigits)
		...
		if (defaultValue != null) {
			if (defaultValue instanceof String) {
				...
				defaultValue = ((String) defaultValue).replaceFirst("'\\s*$", "'"); // sometimes
				// oracle adds an extra space after the trailing ' (see
	...
	public DatabaseSnapshot createDatabaseSnapshot(String schema, Set<DiffStatusListener> statusListeners)
	...
	public String getColumnType(String columnType, Boolean autoIncrement) {
		if (columnType.startsWith("java.sql.Types.VARCHAR")) {
			columnType = columnType.replace("java.sql.Types.VARCHAR", "VARCHAR2");
		}
		return super.getColumnType(columnType, autoIncrement);
	}
