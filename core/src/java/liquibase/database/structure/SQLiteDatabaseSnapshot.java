...
public class SQLiteDatabaseSnapshot extends SqlDatabaseSnapshot {
     ...
     * Creates a snapshot of the given database.
     */
    public SQLiteDatabaseSnapshot(Database database, Set<DiffStatusListener> statusListeners, String requestedSchema)
    ...
    protected void readTablesAndViews(String schema) throws SQLException, JDBCException {
        ...
        ResultSet rs =
                databaseMetaData.getTables(database.convertRequestedSchemaToCatalog(schema),
            ...
            if ("TABLE".equals(type)) {
                ...
                try {
                    ...
                    System.out.println("Error getting view with "
                            + ((AbstractDatabase) database).getViewDefinitionSql(schema, name));
                    throw e;
                }
    ...
    protected void readPrimaryKeys(String schema) throws JDBCException, SQLException {
        ...
        for (Table table : tablesMap.values()) {
            ResultSet rs =
                    databaseMetaData.getPrimaryKeys(database.convertRequestedSchemaToCatalog(schema),
    ...
    protected void readColumns(String schema) throws SQLException, JDBCException {
        ...
        if (database instanceof SQLiteDatabase) {
            ...
            for (Table cur_table : tablesMap.values()) {
                ...
                ResultSet rs =
                        databaseMetaData.getColumns(database.convertRequestedSchemaToCatalog(schema),
        ...
        for (Table table : tablesMap.values()) {
            ...
            while ((rs != null) && rs.next()) {
                ...
                while ((rs_2 != null) && rs_2.next()) {
                    ...
                    if (index_unique) {
                        Column column = columnsMap.get(table.getName() + "." + index_column_name);
    ...
    protected void readSequences(String schema) throws JDBCException {
        ...
        if (database.supportsSequences()) {
            ...
            List<String> sequenceNamess =
                    (List<String>) database.getJdbcTemplate().queryForList(database.createFindSequencesSQL(schema),
